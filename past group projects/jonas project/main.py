...
#Items
[13, 15], 8, "1", [39, 56], 8, "2", [16, 75], 8, "3", [39, 86], 8, "4",
#Enemies
...
def choices(): #Every item in the game that can be both picked up and used. Gives the action you can do with the item, and what that action does.
    ...
    # Swords (Attack Items)
    if '1' in inventory:
        choiceReturns.append(['1', 'attack', 'You swing the Stone Sword with force!', 10])
    if '2' in inventory:
        choiceReturns.append(['2', 'attack', 'You slash with the Iron Sword, dealing significant damage!', 15])
    if '3' in inventory:
        choiceReturns.append(['3', 'attack', 'You quickly stab with the Shortsword, a precise strike!', 12])
    if '4' in inventory:
        choiceReturns.append(['4', 'attack', 'You swing the Platinum Sword, unleashing devastating power!', 25])

    # Shields (Defend Items)
    if '1' in inventory:
        choiceReturns.append(['1', 'defend', 'You raise the Wooden Shield, blocking incoming damage.', 5])
    if '2' in inventory:
        choiceReturns.append(['2', 'defend', 'You brace with the Iron Shield, significantly reducing harm.', 10])
    if '3' in inventory:
        choiceReturns.append(['3', 'defend', 'You fortify yourself with the Steel Shield, absorbing heavy blows.', 15])
    if '4' in inventory:
        choiceReturns.append(['4', 'defend', 'You wield the Platinum Shield, deflecting most attacks.', 20])

    # Healing Potions (Heal Items)
    if '1' in inventory:
        choiceReturns.append(['1', 'heal', 'You drink a Small Healing Potion, feeling a bit better.', 15])
    if '2' in inventory:
        choiceReturns.append(['2', 'heal', 'You drink a Medium Healing Potion, regaining health.', 30])
    if '3' in inventory:
        choiceReturns.append(['3', 'heal', 'You consume a Large Healing Potion, restoring substantial health.', 50])
    if '4' in inventory:
        choiceReturns.append(['4', 'heal', 'You drink the Elixir of Life, fully revitalizing yourself!', 100])
